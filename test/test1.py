...
class Seiko:
    ...
    def run(self):
        ...
        while True:
            ...
            # new_points=self.rotation(xys-[cx,cy],angle).astype(np.int32)
            # new_points+=[cx,cy]
            new_points=self.rotationx(xys,cx,cy,angle).astype(np.int32) #计算原轮廓内坐标点
            ...
            points_new=new_points.view([('', new_points.dtype)] * new_points.shape[1])
            miss_point=np.setdiff1d(points_all, points_new).view(xys.dtype).reshape(-1,2)

            # empty_oricor=self.rotation(miss_point-[cx,cy],-angle)
            # value=self.linear_interpolation(empty_oricor+[cx,cy])
            empty_oricor=self.rotationx(miss_point,cx,cy,-angle)# 将找到的差集在反向旋转到原图
            value=self.linear_interpolation(empty_oricor) # 对这些坐标点双线性插值

            mask[new_points[:,1],new_points[:,0]]=self.img[ys,xs] # 将整数点像素值映射
    ...
    def rotationx(self,xys,cx,cy,angle):
        ...
        xys=xys-[cx,cy] # 不能-=这种增量写法 会改变xys的数值 从而影响别的函数
